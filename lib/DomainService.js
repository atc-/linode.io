...
var lookupIPAndUpdateDomain = (domain, subdomain) => {
  return this.getExternalIP().then(ip => {
    l.findAndUpdateDomain(domain, subdomain, ip);
  });
};
...
class DomainService {
  constructor(apiKey, options) {
    ...
    this.lookupService = options && options.lookupServiceURL != '' && options.lookupServiceURL ? options.lookupServiceURL : 'www.icanhazip.com';
  }
  ...
  findAndUpdateDomain(domain, subdomain, ip) {
    return this.findDomain(domain).then(d => {
      return this.getDomain(d.DOMAINID).then(domain => {
        return { domainId: d.DOMAINID, domains: domain };
      });
    }).then(dom => {
      return { domainId: dom.domainId, sub: dom.domains.DATA.filter(i => {
          return i.NAME === subdomain;
        })[0] };
    }).then(subDef => {
      return this.updateDomain(subDef.domainId, subDef.sub.RESOURCEID, ip);
    });
  }
